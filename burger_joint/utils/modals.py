...
class EditingMenuItemModal(Modal):
	def __init__(self, player: Player, menu_item_index: int) -> None:
		super().__init__(title='Edit Item from menu')
		
		self.player: Player = player
		self.menu_item_index: int = menu_item_index
		player_menu_item: MenuItem = player.menu_items[menu_item_index]
		self.food_item_ID: FoodItemID = \
			player_menu_item.food_item_ID
		
		self.add_item(InputText(label='Name', value=player_menu_item.name))
		self.add_item(
			InputText(
				label='Price',
				value=str(player_menu_item.price),
...
class SettingAddedMenuItemModal(Modal):
	def __init__(self, player: Player, food_item_id: FoodItemID) -> None:
		...
		self.add_item(InputText(label='Name', value=self.food_item.name))
		self.add_item(
			InputText(
